...
export function Button({
  ...
  return (
    <button
      className={clsx(
        "inline-flex items-center justify-center gap-2 rounded-lg px-3.5 py-2 text-sm font-semibold transition duration-200 focus:outline-none focus:ring-4 disabled:cursor-not-allowed disabled:opacity-50",
        variant === "primary" && "bg-clinic-700 text-white shadow-sm shadow-clinic-700/15 hover:-translate-y-0.5 hover:bg-clinic-800 focus:ring-clinic-100",
        variant === "secondary" && "border border-slate-200 bg-white text-slate-700 shadow-sm hover:-translate-y-0.5 hover:border-clinic-200 hover:bg-clinic-25 focus:ring-clinic-100",
...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className={clsx("w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 hover:border-slate-300 focus:border-clinic-500 focus:ring-4 focus:ring-clinic-100", props.className)} />;
}

export function Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return <select {...props} className={clsx("w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition hover:border-slate-300 focus:border-clinic-500 focus:ring-4 focus:ring-clinic-100", props.className)} />;
}

export function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...props} className={clsx("w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 hover:border-slate-300 focus:border-clinic-500 focus:ring-4 focus:ring-clinic-100", props.className)} />;
}
...
export function StatCard({ title, value, icon, tone = "blue" }: { title: string; value: ReactNode; icon: ReactNode; tone?: "blue" | "green" | "amber" | "rose" | "slate" }) {
  const tones = {
    blue: "bg-clinic-50 text-clinic-700 ring-1 ring-clinic-100",
    green: "bg-care-50 text-care-700 ring-1 ring-care-100",
...
export function Badge({ status }: { status: AppointmentStatus | "Active" | "Inactive" | "AVAILABLE" | "NOT AVAILABLE" }) {
  const map: Record<string, string> = {
    ...
    IN_CONSULTATION: "bg-clinic-50 text-clinic-700 ring-clinic-200",
    COMPLETED: "bg-care-50 text-care-700 ring-care-200",
  ...
  return <span className={clsx("inline-flex rounded-full px-2.5 py-1 text-xs font-black tracking-wide ring-1", map[status])}>{status.includes("_") ? statusLabel(status as AppointmentStatus) : status}</span>;
}

export function Modal({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-auto rounded-lg bg-white shadow-clinical ring-1 ring-slate-200/70">
...
export function EmptyState({ title, body }: { title: string; body: string }) {
  return (
    <div className="rounded-lg border border-dashed border-clinic-200 bg-clinic-25 p-8 text-center">
      <Info className="mx-auto text-clinic-500" />
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: rows }).map((_, index) => (
        <div key={index} className="h-12 animate-pulse rounded-lg bg-gradient-to-r from-slate-100 via-slate-50 to-slate-100" />
      ))}
    </div>
  );
}
...
export function Toast() {
  ...
  return (
    <div className="fixed right-5 top-5 z-[60] flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-clinical">
